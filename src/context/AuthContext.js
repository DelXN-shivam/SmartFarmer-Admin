...
export function AuthProvider({ children }) {
  ...
  const checkAuthStatus = () => {
    try {
      ...
      const taluka = localStorage.getItem("Taluka"); // Get taluka from localStorage
      const rememberMe = localStorage.getItem("rememberMe") === "true";

      if (token && role && rememberMe) {
        setUser({ role, token, email, taluka }); // Include taluka in user object
      } else {
  ...
  const clearAuthData = () => {
    ...
    localStorage.removeItem("Taluka"); // Remove taluka too
  ...
  const login = (userData) => {
    setUser(userData);
    // Store taluka in localStorage if available
    if (userData.taluka) {
