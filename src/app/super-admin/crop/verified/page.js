...
export default function VerifiedCropsPage() {
  ...
  // ✅ FILTER: Only verified crops with district & taluka filter
  useEffect(() => {
    if (crops.length > 0) {
      ...
      const filtered = crops.filter((crop) => {
        ...
        // Only verified crops
        if (crop.applicationStatus !== 'verified') return false
      ...
      console.log("✅ VERIFIED CROPS - Total:", crops.length, "Filtered:", filtered.length);
    ...
    <div className="p-4 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-center items-start md:items-center mb-8 gap-10">
        <h1 className="text-3xl font-bold text-gray-900">
          {user?.taluka ? `${user.taluka} Verified Crops` : "Verified Crops"} ({filteredCrops.length})
        </h1>
